    ...
    for ind in population:
        ...
        for i in range(n_obj):
            ...
            if f_maxs[i] - f_mins[i] > 1e-12:
                ind.data['F_norm'][i] = (ind.F[i] - f_mins[i]) / (f_maxs[i] - f_mins[i])
            else:
                ind.data['F_norm'][i] = 0.0
            assert np.isfinite(ind.data['F_norm'][i]), f"Non-finite F_norm encountered in normalization: {ind.F_norm[i]}"
        ind.data['F_norm'] = np.clip(ind.data['F_norm'], a_min=0.0, a_max=1.0)
...
def r2(population, weights, z_ref):
    ...
    for w in weights:
        min_diff = float('inf')
        for p in population:
            max_diff = max([w_j * abs(p.data['F_norm'][j] - z_ref[j]) for j, w_j in enumerate(w)])
...
def get_dynamic_r2_reference(population):
    ...
    max_f = 0
    for i in range(n_obj):
        max_f_i = max([ind.data['F_norm'][i] for ind in population])
        min_f_i = min([ind.data['F_norm'][i] for ind in population])
